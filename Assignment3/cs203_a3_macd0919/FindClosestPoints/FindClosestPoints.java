...
public class FindClosestPoints{
    ...
    public static void main(String[] args)
    {
        ...
        if(getPoints(inputFile))
        {
            ...
            //-1 so index out of bounds does not occur
            ...
            if(mergeSort(setOfPoints))
            {
                ...
                //set min distance to initial value     
                minPoints.setDistance(dist(xSetOfPoints[0],xSetOfPoints[1]));
                minPoints.setPoints(xSetOfPoints[0].getXPoint(), 
                                    xSetOfPoints[0].getYPoint(),
                                    xSetOfPoints[1].getXPoint(),    
                                    xSetOfPoints[1].getYPoint());
                ...
                System.out.println("You need atleast 2 points.");
        }
        else
    ...
    {
       float minDis;//value to hold minDis of either side
       ...
       else
       {
           ...
           int midPoint = xSortedArray[halfOne].getXPoint();
           ...
           boolean done = true;
           int qIndex = 0;
           //check if point should be on left or right side
           //this uses the mid point to determine which side.
           while(qIndex < ySortedArray.length && done)
           {
               //if either index is done copy rest to other side
               if(leftIndex < halfOne && rightIndex < halfTwo)
               {            
                   //check if current point is less or equal to mid point
                   //put it in left array if so
                   if(ySortedArray[qIndex].getXPoint() <= midPoint)
                       ySortLeft[leftIndex++] = ySortedArray[qIndex];
                   //put it in right if not
                   else
                       ySortRight[rightIndex++] = ySortedArray[qIndex];
                   qIndex++;
               }
               else
               {
           ...
           //check which array is not empty if any.
           if(leftIndex == halfOne)
               System.arraycopy(ySortedArray, qIndex, ySortRight,
                                   rightIndex, (halfTwo-rightIndex)); 
           else if(rightIndex == halfTwo)
               System.arraycopy(ySortedArray, qIndex, ySortLeft, 
                                      leftIndex, (halfOne-leftIndex)); 
           ...
           TwoDPoints stripArray[] = new TwoDPoints[numOfPoints];
           int stripArrayCounter = 0;
           //get all points that could have smaller distance from the two halfs
           for(int index = 0; index < ySortedArray.length; index++)
           {
               if((Math.abs(ySortedArray[index].getXPoint() - middle) < 
                                                                       minDis))
               {
                   stripArray[stripArrayCounter] = ySortedArray[index];
                   stripArrayCounter++;
               }
           }
           //check if any of the points are a closer pair
           for(int index = 0; index < stripArrayCounter - 1; index++)
           {
               int nextPoint = index + 1;
               while( (nextPoint <= (stripArrayCounter-1)) && 
                    ((stripArray[nextPoint].getYPoint() - 
                                            stripArray[index].getYPoint()) < 
                                                                       minDis))
               {
                   ClosestPoints testPoints = new ClosestPoints
                               (stripArray[nextPoint].getXPoint(),
                                       stripArray[nextPoint].getYPoint(), 
                                       stripArray[index].getXPoint(),
                                       stripArray[index].getYPoint());
                   testPoints.setDistance(dist(stripArray[nextPoint],
                                                       stripArray[index]));
                   if(testPoints.compareTo(minPoints) == -1)
                   {
                       minPoints = testPoints;
                       minDis = minPoints.distance;
                   }
                   nextPoint++;
               }
           }  
    ...
    public static float bruteClosestPair(TwoDPoints[] pointsArray)
    {
        //initialize closest distance to the first 2 pairs
            ...
            {
                for(int indexTwo = indexOne + 1; indexTwo < 
                                            pointsArray.length; indexTwo++)
                {
                    //check if new pair are closer
                    if (dist(pointsArray[indexOne],pointsArray[indexTwo]) < min 
                                            ||  min < minPoints.getDistance())
                    {
                        //make new min distance if they are
                        min = dist(pointsArray[indexOne], 
                                                        pointsArray[indexTwo]);
                        if(min < minPoints.getDistance())
                        {
                            minPoints.setPoints
                                    (pointsArray[indexOne].getXPoint(), 
